...
/**
 * This class is emulating an embedded tomcat configured with Mobicents Sip Servlets extension
...
public class SipEmbedded {		
	private static final Logger log = Logger.getLogger(SipEmbedded.class.getName());
	 ...
	 * 
	 */
	public SipEmbedded(String serverName) {
...
//		PropertyConfigurator.configure(loggingFilePath);		
        ...
        System.setProperty("javax.servlet.sip.ar.spi.SipApplicationRouterProvider", DummyRouterProvider.class.getCanonicalName());
		sipService = (SipStandardService) Class.forName(SipStandardService.class.getName()).newInstance();
...
//		httpProtocolHandler.setMinSpareThreads(75);
	...
	/**
	 * Allow addition of sip connector
	 */
	...
	public void stopTomcat() {
		...
		if(sipService != null) {			
			try {
				...
				log.log(Level.SEVERE, "SipService already stopped ", e);
			}		
    ...
    private ClassLoader createClassLoader(String name, ClassLoader parent)
        ...
        while (tokenizer.hasMoreElements()) {
            ...
            if (replace && log.isLoggable(Level.FINE))
                log.log(Level.FINE,"Expanded " + before + " to " + replace);
...
//        return System.getProperty("catalina.base", getCatalinaHome());
    ...
    public static ClassLoader createClassLoader(String locations[],
        ...
        if (locations != null && types != null && locations.length == types.length) {
            for (int i = 0; i < locations.length; i++)  {
                ...
                if ( types[i].equals(IS_URL) ) {
                    ...
                    if (log.isLoggable(Level.FINE))
                        log.log(Level.FINE,"  Including URL " + url);
                    list.add(url);
                    ...
                    if (log.isLoggable(Level.FINE))
                        log.log(Level.FINE,"  Including directory " + url);
                    list.add(url);
                    ...
                    URL url = file.toURL();
                    if (log.isLoggable(Level.FINE))
                        log.log(Level.FINE,"  Including jar file " + url);
                    list.add(url);
                    ...
                    if (!directory.exists() || !directory.isDirectory() ||
                        ...
                        continue;
                    if (log.isLoggable(Level.FINE))
                        log.log(Level.FINE,"  Including directory glob "
                    ...
                    for (int j = 0; j < filenames.length; j++) {
                        ...
                        if (!file.exists() || !file.canRead())
                            continue;
                        if (log.isLoggable(Level.FINE))
                            log.log(Level.FINE,"    Including glob jar file "
        ...
        if (log.isLoggable(Level.FINE))
            for (int i = 0; i < array.length; i++) {
                log.log(Level.FINE,"  location " + i + " is " + array[i]);
            }
    ...
    protected void initDirs() {
        ...
        if (temp == null || (!(new File(temp)).exists())
            ...
            log.log(Level.SEVERE, "no temp directory"+ temp);
        }
